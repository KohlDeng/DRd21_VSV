...
/*
bool judgeEndPoint(state_,&trajectory_){
  //skipad doesn't need lap_count
  //trackdrive does need it!!!
  if(lap_count <= 3 && mission == "track_drive"){
    return false;
  }  

  double delta_x;
  double delta_y;
  double dist_min = 0.5; //0.5m is param to be set
  delta_x = state_.x - map_traj(last).x;
  delta_y = state_.y - map_traj(last).y;
  dist_to_EndPoint = hypot(delta_x,delta_y);
  if(dist_to_EndPoint <= dist_min){
    return true;
    // "true" indicate that vehicle is quite near EndPoint, vehicle should stop right now!!!
  }
}
*/
